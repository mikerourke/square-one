...
/**
 * Represents a table with the associated child components.
 * @export
 * @class Table
 */
class Table extends Component<DefaultProps, Props, State> {
  ...
  /**
   * Fires the handleSortOrderChange function (passed from props) when the
  ...
  handleHeaderColumnClick = (
    ...
    // Accommodate for the hidden ID column.
    const adjustedColumnIndex = (columnIndex - 2);

    // If the specified column is sortable, apply the sort based on the
    // selected column and order.
            ...
            <Row onCellClick={this.handleHeaderColumnClick}>
              ...
              {columns.map((row, index) => (
                <HeaderColumn
                  ...
                  key={index}
                  order={sort.column === row.key ? sort.order : 'asc'}
                  sortable={row.sortable}
                  sorted={sort.column === row.key}
                  style={Object.assign({}, { fontWeight: 600 },
                    row.style || {})}
                  tooltip={row.tooltip}
                >
                  <span>{row.label}</span>
              ...
              <Row key={rowIndex}>
                ...
                >
                  <IconButton
                    ...
                    onTouchTap={() => handleRowIconTouchTap(row)}
                  >
                    mode_edit
                  </IconButton>
                </RowColumn>
